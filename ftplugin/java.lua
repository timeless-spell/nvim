...
local java_debug = vim.fn.glob(
	vim.env.HOME
		.. '/.jdt-ls/java-debug-0.53.1/com.microsoft.java.debug.plugin/target/com.microsoft.java.debug.plugin-*.jar',
	true
)
local java_test = vim.fn.glob(
	vim.env.HOME
		.. '/.jdt-ls/vscode-java-test-0.43.2/java-extension/com.microsoft.java.test.plugin/target/com.microsoft.java.test.plugin-*.jar',
	true
)
...
config['on_attach'] = function (client, bufnr)
	...
	local keymaps = {
		{
			keys = '<leader>jo',
			func = "<cmd>lua require('jdtls').organize_imports()<cr>",
			desc = '[JDTLS] Organize Imports',
			buffer = bufnr,
		},
		{
			keys = '<leader>je',
			func = "<cmd>lua require('jdtls').extract_variable()<cr>",
			desc = '[JDTLS] Extract Variable',
			buffer = bufnr,
		},
		{
			keys = '<leader>je',
			func = "<cmd>lua require('jdtls').extract_variable(true)<cr>",
			desc = '[JDTLS] Extract Variable',
			mode = 'v',
			buffer = bufnr,
		},
		{
			keys = '<leader>jE',
			func = "<cmd>lua require('jdtls').extract_constant()<cr>",
			desc = '[JDTLS] Extract Constant',
			buffer = bufnr,
		},
		{
			keys = '<leader>jE',
			func = "<cmd>lua require('jdtls').extract_constant(true)<cr>",
			desc = '[JDTLS] Extract Constant',
			mode = 'v',
			buffer = bufnr,
		},
		{
			keys = '<leader>jM',
			func = "<cmd>lua require('jdtls').extract_method(true)<cr>",
		...
		{
			keys = '<leader>jt',
			func = "<cmd>lua require('jdtls').test_class()<cr>",
			desc = '[JDTLS] Test Class',
			buffer = bufnr,
		},
		{
			keys = '<leader>jt',
			func = "<cmd>lua require('jdtls').test_nearest_method()<cr>",
